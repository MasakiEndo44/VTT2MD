...
1.  [Releases ページ](https://github.com/MasakiEndo44/VTT2MD/releases) にアクセスします。
2.  最新バージョンの「Assets」から **`VTT2MD_Setup.exe`** をダウンロードします。
3.  ダウンロードした `VTT2MD_Setup.exe` をダブルクリックして実行します。
4.  画面の指示に従ってインストールを完了します。
5.  スタートメニューまたはデスクトップのショートカットからアプリを起動できます。

### 使い方

アプリケーションを起動したら、ウィンドウ内にVTTファイルをドラッグ＆ドロップするか、「ファイルを選択」ボタンからファイルを選んで変換を開始してください。
